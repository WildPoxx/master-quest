...
// V13, 0.30.3-v13.6 (Mario, 2026-09-15): esta linha tinha cinco skins e passa a ter as dez
// da linha 14. Duas diferencas deliberadas, explicadas em src/foundry/skin-settings.js: a
// skin espacial passa a se chamar `cosmic`, como na 14, com mapa de id antigo para nao
// derrubar mundo que ja a tinha gravada como `cosmere`; e o padrao da linha continua
...
test("skin desconhecida e reescrita no mundo, e nao so corrigida na tela", () => {
  ...
  assert.equal(result.skin, INTERFACE_SKINS.modernInvestigation);
  assert.deepEqual(gravados, [["master-quest", "interfaceSkin", INTERFACE_SKINS.modernInvestigation]]);
});
...
test("normalizar aceita as nove e recusa o resto", () => {
  ...
  for (const lixo of ["", null, undefined, "skin-que-nunca-existiu", 42]) {
    assert.equal(normalizeInterfaceSkin(lixo), INTERFACE_SKINS.modernInvestigation);
  }
});

/* --- 5. o id antigo desta linha ------------------------------------------------------- */

test("v13.6: mundo que tinha a Cosmere escolhida acorda na Cosmic, e nao no padrao", () => {
  // A troca de nome nao pode custar a escolha de ninguem. Traduzir o id antigo e o unico
  // jeito de renomear sem que a campanha em curso perca a propria skin no meio do caminho.
  assert.equal(normalizeInterfaceSkin("cosmere"), INTERFACE_SKINS.cosmic);
  assert.notEqual(normalizeInterfaceSkin("cosmere"), INTERFACE_SKINS.modernInvestigation);

  const gravados = [];
  const game = {
    user: { isGM: true },
    settings: { get: () => "cosmere", set: (...args) => gravados.push(args) }
  };
  const result = healStoredInterfaceSkin({ game });

  assert.equal(result.status, "healed");
  assert.equal(result.skin, INTERFACE_SKINS.cosmic);
  assert.deepEqual(gravados, [["master-quest", "interfaceSkin", INTERFACE_SKINS.cosmic]],
    "o valor novo tem de ir para o banco: consertar so na tela deixaria o menu vazio");
});
